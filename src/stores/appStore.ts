...
export interface ChatMessage {
  ...
  timestamp: Date
  context?: AIContext[]
  reasoning?: string  // 思考过程内容（R1/K2 等模型）
}
...
export interface AppState {
  ...
  appendToMessageReasoning: (id: string, reasoningChunk: string) => void
...
export const useAppStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      appendToMessageReasoning: (id, reasoningChunk) => set((state) => ({
        chatHistory: state.chatHistory.map(m => m.id === id ? { ...m, reasoning: (m.reasoning || '') + reasoningChunk } : m)
      })),
